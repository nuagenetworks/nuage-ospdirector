...
def virt_customize(command):
    return cmds_run(['virt-customize --run-command %s' % command])

def virt_customize_run(command):
    return cmds_run(['virt-customize --run %s' % command])

def virt_copy(command):
    return cmds_run(['virt-copy-in -a %s' % command])
...
def rhel_subscription(username, password, pool, image):
    cmds_run(['cat <<EOT > rhel_subscription \n'
...
def add_files (image, version, workingDir):
    version = int(version)
    if  version == 7:
        virt_customize('"mkdir -p /etc/puppet/modules/nuage/manifests/7_files" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_copy('%s %s/7_files/neutron_plugin_nuage.rb /etc/puppet/modules/nuage/manifests/7_files' % (image, workingDir))
        ...
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/7_files/neutron_plugin_nuage.rb /etc/puppet/modules/neutron/lib/puppet/type/neutron_plugin_nuage.rb" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/7_files/ini_setting.rb /etc/puppet/modules/neutron/lib/puppet/provider/neutron_plugin_nuage/ini_setting.rb" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/7_files/impl_ifcfg.py /usr/lib/python2.7/site-packages/os_net_config/impl_ifcfg.py" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))

    if version == 8:
        virt_customize('"mkdir -p /etc/puppet/modules/nuage/manifests/8_files" - a %s - -memsize %s - -selinux - relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_copy('%s %s/8_files/neutron_plugin_nuage.rb /etc/puppet/modules/nuage/manifests/8_files' % (image, workingDir))
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/8_files/neutron_plugin_nuage.rb /etc/puppet/modules/neutron/lib/puppet/type/neutron_plugin_nuage.rb" - a %s - -memsize %s - -selinux - relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))

    if version == 9:
        virt_customize('"mkdir -p /etc/puppet/modules/nuage/manifests/9_files" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_copy('%s %s/9_files/nuage.pp /etc/puppet/modules/nuage/manifests/9_files'  % (image, workingDir))
        virt_copy('%s %s/9_files/neutron-server.service /etc/puppet/modules/nuage/manifests/9_files'  % (image, workingDir))
        ...
        virt_copy('%s %s/9_files/topology-collector-4.7.0.tar.gz /etc/puppet/modules/nuage/manifests/9_files'  % (image, workingDir))
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/9_files/nuage.pp /etc/puppet/modules/neutron/manifests/plugins/nuage.pp" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/9_files/neutron-server.service /usr/lib/systemd/system/neutron-server.service" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/9_files/ml2.pp /etc/puppet/modules/neutron/manifests/plugins/ml2.pp" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/9_files/filter.pp /etc/puppet/modules/nova/manifests/scheduler/filter.pp" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/9_files/grub /etc/default/grub" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/9_files/sriov.pp /etc/puppet/modules/neutron/manifests/agents/ml2/sriov.pp" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/9_files/topology-collector-4.7.0.tar.gz /root/topology-collector-4.7.0.tar.gz" -a %s --memsize %s --selinux-relabe' %( image, VIRT_CUSTOMIZE_MEMSIZE))
    ...
    if version == 10:
        ...
        virt_copy('%s %s/10_files/neutron-server.service /etc/puppet/modules/nuage/manifests/10_files'  % (image, workingDir))
        ...
        virt_customize('"cp /etc/puppet/modules/nuage/manifests/10_files/neutron-server.service /usr/lib/systemd/system/neutron-server.service" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
...
def uninstall_packages(image, version):
    ...
    if version <= 9 :
        virt_customize('"yum remove python-openvswitch -y" -a %s --memsize %s --selinux-relabel' %( image, VIRT_CUSTOMIZE_MEMSIZE))
...
def main(args):
    ...
    if len(args) < 3:
        ...
        libguestfs = cmds_run(['rpm -q --quiet libguestfs-tools-c'])
        if libguestfs and libguestfs.strip():
        ...
        if 'RhelUserName' in argsDict and 'RhelPassword' in argsDict and 'RhelPool' in argsDict:
            ...
            rhel_subscription(argsDict['RhelUserName'][0],argsDict['RhelPassword'][0],argsDict['RhelPool'][0],argsDict['ImageName'][0])
